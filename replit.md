...
- **Royalty Accounting System**: Financial engine for statement ingestion, asset matching, royalty calculation, advance recoupment, and payment management, supporting multi-currency and PRO statements. Supports CSV, Excel (.xlsx/.xls), and PDF file uploads — PDFs are parsed via pdfplumber table extraction with OpenAI fallback for non-tabular layouts. Includes a professional royalty processing pipeline with matching, allocation preview, audit-safe reprocessing, and payables management.
