...
- **Rights & Contract Tracking**: Supports deal-level contracts with parties, territories, advances, and asset-to-contract linking. Per-asset rights splits are validated to 100%. Song-level Publishing % and Master % are read-only (derived from credit-level splits only; backend rejects direct writes to these fields). Legacy manual values shown with migration prompt.
- **Multi-Client Song Grouping**: Songs shared across multiple clients link via `shared_song_group_id` UUID. SongDetailModal shows "Also represented for: [Client B]" badges and supports Link/Unlink actions. Group/ungroup API endpoints at `POST /api/songs/group` and `DELETE /api/songs/{id}/group`.
- **Auto-Create Creator**: Adding a credit with `new_creator_name` (instead of `creator_id`) auto-creates a Creator record in `credits.py`.
- **MLC Statement Processing**: MLC (Mechanical Licensing Collective) detected as a PRO source type. MLC statements trigger multi-client royalty distribution via `shared_song_group_id`, normalizing split shares when total exceeds 100%. Non-MLC statements keep single-client behavior.
...
- **Catalog Refactor — Released/Unreleased Model**: Sidebar shows "Unreleased" nested under "Catalog" (routes to WorksPage at `/catalog/unreleased`; old `/works` route redirects). Catalog page tabs are [All | Released | Unreleased] with entry_type filter chips (Song/Instrumental/Remix/Sample/Demo) in the Unreleased view. Songs have `release_status` (unreleased/released, default unreleased), `entry_type` (Song/Instrumental/Remix/Sample/Demo), and `parent_song_id` (FK to songs for duplicate tracking). Daily 00:05 UTC scheduled job auto-flips songs to "released" when `release_date` arrives. "Mark as Released" button on SongDetailModal for manual override. Duplicated songs reset to unreleased with parent link shown in detail view.
- **Duplicate Catalog Entry**: Quick song duplication feature with `parent_song_id` linking back to original and `release_status` reset to unreleased.
