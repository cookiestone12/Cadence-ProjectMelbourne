...
- **Catalog Valuation Tool**: Utilizes a weighted average of industry metrics and a proprietary Black Box Algorithm, including an underwriting engine for statement-driven valuations. The **source-typed valuation engine** (`compute_source_typed_valuation`) annualizes matched royalty statement lines by source type (PRO / MLC / DSP / SoundExchange) using the per-source cadence in `config/statement_formats.py`, buckets revenue into performance / mechanical / sync / streaming with industry multipliers (10× / 9× / 7× / 12.5×), and splits the resulting catalog value into artist (MASTER) and publisher (PUBLISHING) shares from `RightsSplit` rows (defaults to 50/50 when no splits exist). Persists per-song rows with `valuation_method='SOURCE_TYPED'`. Exposed at `POST /api/valuation/source-typed/run` and `GET /api/valuation/source-typed/summary` and surfaced as a "Revenue by Source" panel on the Valuation page that honors the per-creator scope.
...
- **Phase 2 — Clean House Refactor (Task #169)**:
  - **Legacy pages removed**: `frontend/src/pages/{Search,CatalogPage,CatalogView}.jsx` and `components/Navigation.jsx` deleted; `App.jsx` already routed to `NewCatalogPage` and `SearchPage`.
  - **Sidebar regrouped** into collapsible sections (Catalog / Financials / Analytics / Team / Settings) in `Sidebar.jsx`. Catalog & Financials default-expanded; the section containing the active route auto-expands. Settings is now its own collapsible section (no longer a standalone bottom link).
  - **`/api/v1/` true namespacing**: `_mount_v1_routes()` in `backend/main.py` walks `app.router.routes` after every router has been included and registers a real `APIRoute` clone of each `/api/...` path under `/api/v1/...`. Both prefixes appear as first-class routes in OpenAPI / `/docs` and round-trip through validation, dependency injection, and exception handling identically. 448 legacy routes → 448 mirrored v1 routes. Frontend wires this up through `frontend/src/lib/apiBase.js` (`API_BASE = '/api/v1'`, `apiUrl(path)` helper) plus a global axios request interceptor in `App.jsx` that pipes every request URL through `apiUrl`. The 4 raw `fetch()` callers (AdminDashboard, AssistantChat) also import `apiUrl`. `/api/internal/portal/*` paths are intentionally excluded from rewriting because their auth cookie is path-scoped to `/api/internal`.
  - **`models.py` split into 14 domain modules** under `backend/models/` (`users / organizations / creators / catalog / works / releases / contracts / royalties / financials / analytics / integrations / notifications / sharing / misc`). The original `models.py` (2655 lines, 140 classes) is now a 16-line shim that does `from .{domain} import *` for backwards compatibility. `__init__.py` re-exports everything from the focused modules.
  - **Advance v1 → v2 consolidation**: Alembic revision `b7c8d9e0f1a2` migrates rows from the legacy `advances` table into `advance_pools` (auto-creating Payee rows per orphaned creator), drops the legacy table, renames `advance_pools` → `advances`, and re-points `royalty_ledger_entries.advance_id` FK at the renamed table. The SQLAlchemy `AdvanceV2` class is renamed to `Advance` (with a backwards-compat `AdvanceV2 = Advance` alias kept in `models/royalties.py`); the legacy v1 `Advance` class is removed. The four legacy v1 routes (`GET/POST/PATCH/DELETE /api/royalties/advances/{org_id}[/{advance_id}]`) and the contract-confirmation auto-advance writer in `routes/royalties.py` were rewritten as a translation layer that takes the v1 JSON contract (`creator_id`, `description`, `amount_cents`, `recouped_cents`) and reads/writes the unified `advances` table — frontend RoyaltiesPage / CreatorDetailPage / CreatorAccountingEnhanced continue to work unchanged.
  - **Schema realignment to clear pre-existing drift**: Models updated to match the actual DB so `alembic check` returns zero ops. JSONB columns (`audit_logs.details`, `creators.custom_links`, `royalty_statements.reconciliation_details`) declared as `JSONB().with_variant(JSON(), 'sqlite')` so SQLite-backed unit tests still compile. `songs.spotify_popularity` switched to `SmallInteger`. New `MigrationLock` model (`models/misc.py`) declares the boot-time migration-serialization table. Functional unique index `idx_users_username_lower` declared in `users.py` __table_args__. Hand-named legacy indexes (`ix_org_members_*`, `ix_placements_organization_id`, `ix_registration_reports_organization_id`, `ix_song_edit_history_organization_id`) added to model `__table_args__`. Alembic revision `c6dddf556d5f` adds the missing `action_items_placement_id_fkey` FK that was declared in the model but absent from the DB.
