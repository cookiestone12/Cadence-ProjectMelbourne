...
- **Phase 2 — Clean House Refactor (Task #169)**:
  ...
  - **Sidebar regrouped** into collapsible sections (Catalog / Financials / Analytics / Team / Settings) in `Sidebar.jsx`. Catalog & Financials default-expanded; the section containing the active route auto-expands.
  - **`/api/v1/` prefix alias**: an `APIVersionRewriteMiddleware` ASGI class in `backend/main.py` rewrites incoming `/api/v1/...` → `/api/...` in scope. Every existing route is now reachable under both prefixes (zero-risk back-compat). Frontend exposes the constant via `frontend/src/lib/apiBase.js` (`API_BASE = '/api/v1'`, `apiUrl(path)` helper) — call sites are not mass-rewritten yet.
