...
- **Production Infrastructure Foundation**: APP_ENV-driven environment separation, `/health` endpoint, hardened CORS, per-request `X-Request-ID` for structured JSON logs, HTTPS enforcement, in-process ring buffer log handler, and global exception handlers. **Deployment target is Reserved VM (not Autoscale)** — Cadence runs five APScheduler jobs (15-min email digests, hourly storage scans, 4-hour chart ingestion, daily 00:05 UTC release flip, nightly 03:00 UTC session cleanup) that need the process alive 24/7. Autoscale scales containers to zero between web requests, which would silently break every scheduled job. The VM target also bypassed an Autoscale promoter incompatibility we hit in April 2026 where promotion repeatedly failed with contradictory AI-generated error messages despite a valid single-port `.replit` config. Build command: `cd frontend && npm run build`. Run command: `gunicorn backend.main:app -c backend/gunicorn_config.py --bind 0.0.0.0:5000`. The FastAPI app serves both the API and the built frontend SPA from `frontend/dist` on the same port.
- **Records Inline Edit Save (Task #131)**: The Roster → Client profile → Records tab inline editor no longer sends `publishing_percentage` / `master_percentage` in the PATCH body — those values are read-only at the song level (derived from credit-level splits) and the backend rejects any payload containing them. The Pub % cell in the inline edit row is now a read-only display with caption "Edit in Rights & Splits". Editing Title, Artist, Label, Released toggle, Spotify link, Advance, PRO/DSP/SX/MLC checkboxes, Paid/Invoiced/Contract Executed, and Notes all save correctly. Pinned by `backend/tests/test_song_patch_no_pub_pct_131.py`.
- **Advances Normalized Across Client Profile (Task #133)**: The Roster → Client profile Records table now correctly renders `song.advance_amount` under the Advance header (previously the cell was missing and `is_invoiced` bled into that slot, which made phantom "$20,000" pills appear on songs whose advance was actually NULL). The orphan `is_registered_with_dsp` cell with no header was removed. The inline-edit row now exposes a `$` number input bound to `editForm.advance_amount` so users have a place to type advances; `saveEdit` already round-trips it as cents. The page header pill ("$X advances") and Overview → Financials → Total Advances now read from a single unified rollup that sums song-level + creator-level (`Advance` table) + contract-level (incoming `Contract.advance_amount_cents`) advances, sourced from `GET /api/royalties/creator-accounting/{org_id}/{creator_id}` which is loaded on profile mount. A small caption under the Financials number breaks the total down ("Songs $X · Creator $Y · Contracts $Z").
- **Schedule A Splits Materialization**: Both Schedule A ingestion paths now materialize splits the same way: the legacy `schedule_a_ingestion.py` and the Catalog-UI re-upload path `POST /api/csv/import/{org_id}` in `csv_upload.py` (Task #130). Each writes credit-level `pub_share`/`master_share` and creates the matching `Contract` (SPLIT_SHEET) + `ContractAsset` + `RightsSplit` rows via `sync_credit_to_splits`, so Pub %/Master % survive the credit-driven rollup and are visible in both the internal Catalog View and the Client Portal catalog tab. The portal `/api/client-portal/catalog` returns `publishing_percentage`, `master_percentage`, and per-credit `pub_share`/`master_share`. A backfill script (`backend/scripts/backfill_schedule_a_splits_120.py`) materializes splits for songs imported before this fix; the same backfill is exposed for production through super-admin endpoint `POST /api/internal/backfill/schedule-a-splits?dry_run=…&org_id=…` (Task #128) since deployments don't expose a shell. The script's previous broken `SessionLocal` import has been corrected — it now imports from `backend.models.database` directly.
...
- **Health Score Reaches 100% (Task #140)**: The per-song checklist always had ten items (total weight 90), but only eight were wired to the inline editor. The other two — **LG-02 "Publishing splits confirmed"** (weight 10) and **MD-03 "Credits finalized"** (weight 5) — had no UI and never flipped to COMPLETED, capping every song in every org at 75/90 = 83.3% no matter how many fields the user filled in. Even fully-imported Schedule A songs sat at 11.11% (just ISRC + ISWC) until manual edits triggered a sync. `sync_song_to_checklist` in `backend/utils/health_sync.py` now derives both items from data the app already owns: LG-02 = sum of `SongCredit.pub_share` is within 0.5 of 100 (mirroring the credit-driven Pub %/Master % rollup), MD-03 = at least one `SongCredit` has a `creator_id`. Recomputation is wired into every credit/split mutation path: `sync_credit_to_splits`, `credits.create_credit`/`update_credit`/`delete_credit`/`resolve_credit`, and the standalone `add_song_split`/`delete_song_split` endpoints — so adding a credit, editing splits on the Rights & Splits tab, or deleting the last credit all push the score back through the new evaluator. `SongResponse` now exposes `has_credits` and `has_full_pub_splits` booleans (bulk-computed by `_compute_credit_flags_for_songs` in `routes/songs.py` to keep the list endpoint single-query) so the frontend's per-song HealthScoreBreakdown card can show the two new items as missing-field chips. `HEALTH_FIELDS` in `frontend/src/pages/CreatorDetailPage.jsx` gained the two derived items with a new `flag` field type. Backfill script `backend/scripts/backfill_health_scores_140.py` walks every song in every org and re-runs `sync_song_to_checklist`, also seeding any missing `SongChecklistStatus` rows so freshly-imported songs aren't skipped; super-admin endpoint `POST /api/internal/backfill/health-scores?dry_run=…&org_id=…` exposes the same path for production where there's no shell.
