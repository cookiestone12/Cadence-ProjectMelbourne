...
export default function CatalogView() {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      link.href = url
      link.download = `Cadence_Catalog_Report_${catalogSummary.name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`
  ...
  return (
    <div className="min-h-screen bg-[#F5F7F4] p-6 lg:p-8">
      ...
      <div className="bg-white rounded border border-[rgba(59,77,67,0.08)] shadow-lg p-6 mb-6 hover:shadow-lg transition-colors duration-200">
        ...
        <p className="text-sm text-[#7A8580] mb-4">Use the official Cadence Schedule A template only.</p>
        <div
          {...getRootProps()}
