...
class DocumentParseResult:
    ...
    def to_preview_response(self) -> Dict[str, Any]:
        headers = ["title", "primary_artist", "publishing_percentage", "section", "notes"]
        all_songs = []
...
def parse_creator_info(text: str) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    if ipi_match:
        ...
        pro_match = re.search(r'(BMI|ASCAP|SESAC|GMR)\s+IPI', text, re.IGNORECASE)
        if pro_match:
            pro_name = pro_match.group(1).upper()

    id_match = re.search(r'(?:BMI|ASCAP|SESAC|GMR)\s+ID\s*#?\s*:?\s*(\d+)', text, re.IGNORECASE)
    if id_match:
