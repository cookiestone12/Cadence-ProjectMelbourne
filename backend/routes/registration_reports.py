...
def export_registration_csv(
    ...
    if asset_type == "works":
        writer.writerow([
            "Work Title", "ISWC", "Work Type", "Alternate Titles",
        ...
        writer.writerow([
            "Song Title", "Primary Artist", "ISRC", "ISWC", "Release Date",
            "Writer Name", "Writer Legal Name", "Writer Role", "Share %",
            "Writer PRO", "Writer IPI",
            "Publisher Name", "Publisher PRO", "Publisher IPI",
            "Administrator Name", "Admin PRO", "Admin IPI",
        ...
        for song in songs:
            ...
            if not data["writers"]:
                writer.writerow([
                    ...
                    data["iswc"] or "", data.get("release_date") or "",
                    "", "", "", "", "", "", "", "", "", "", "", "",
                    "INVALID: " + "; ".join(data["validation_issues"])
                ])
            for w in data["writers"]:
...
def export_registration_pdf(
    ...
    elements.append(Paragraph(f"Total: {len(items)} | Ready: {valid_count} | Needs Attention: {len(items) - valid_count}", subtitle_style))
    ...
    for item in items:
        ...
        elements.append(Paragraph(f'<b>{title}</b> <font color="#7A8580">({identifier})</font> — <font color="{status_color}">{status}</font>', section_style))
